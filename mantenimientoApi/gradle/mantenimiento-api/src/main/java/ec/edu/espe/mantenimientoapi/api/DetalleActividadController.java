...
public class DetalleActividadController {
    ...
    @GetMapping("detalleActividad/leerPorActividad/{id}")
    public ResponseEntity<List<DetalleActividad>> getDetalleActividadByActividad(@PathVariable Integer id) {
        try {
            return ResponseEntity.ok(this.detalleActividadService.getDetalleActividadByActividad(id));
        } catch (Exception e){
            e.printStackTrace();
            return ResponseEntity.badRequest().build();
        }
    ...
    @DeleteMapping("detalleActividadesPorActividad/borrar/{id}")
    public ResponseEntity<?> deleteAllDetalleActividadByActividad(@PathVariable Integer id){
        try {
            this.detalleActividadService.deleteAllDetalleActividadByActividad(id);
            return ResponseEntity.ok().build();
        } catch (Exception e){
            e.getStackTrace();
            return ResponseEntity.badRequest().build();
